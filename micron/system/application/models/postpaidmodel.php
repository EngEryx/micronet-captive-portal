...
class Postpaidmodel extends model {
	...
	function editAccount(){
		...
		$user_acc['username'] = $_POST['postuser'];
		$user_acc['realname'] =$_POST['fullname'];
		$user_acc['bill_by'] = $_POST['postbillby'];
		$user_acc['smsmobile'] = $_POST['postmobile'];
		$user_acc['password'] = $_POST['postpass'];
		//$user_acc['valid_until'] = "October 30 2015 24:00:00";
		//$user_acc['created_by'] = "Ericg";
		//$user_acc['IdleTimeOut'] = "10";
		$postuser =$_POST['postuser'];
		$postpass =$_POST['postpass'];
		$bw_download = $_POST['bw_download'];
		$bw_upload = $_POST['bw_upload'];
		//fields on postpaid_account table	
		$this->db->where('username', $postuser);
		$this->db->update($this->_table,$user_acc);
		
		//field on radcheck table
		$password = array('value' => $postpass); 
		$this->db->where('username',$postuser);
		...
		//Dowloadrate
		$bw_rate = array('value' => $bw_download);
		$this->db->where('username',$postuser);
		$this->db->where('attribute','WISPr-Bandwidth-Max-Down');
		$this->db->update($this->_table_radreply,$bw_rate);
		
		//Uploadrate
		$bw_upload = array('value' => $bw_upload);
		$this->db->where('username',$postuser);
		...
		//Accounting interval
		if($ci->config->item('postpaid_acct_interim_interval')){
		$change =array('value'=>$ci->config->item('postpaid_acct_interim_interval'));
		$this->db->where('username',$postuser);
		$this->db->where('attribute','Acct-Interim-Interval');
		
		$this->db->update($this->_table_radreply,$change);
		}
		$this->db->trans_complete();
	}
